/* ============================================================
   ...
   Строка BUILD подставляется скриптом сборки (server/deploy.sh).
   Правило простое:
   ...
   С переходом на сборку Vite (Этап 9) имена файлов кода стали
   хэшированными (assets/detail-HASH.js) и меняются при каждой
   сборке — заранее их не перечислить. Поэтому в предзагрузку
   (CORE) попадают только стабильные по имени файлы: сами страницы,
   манифест, иконки. Хэшированные чанки и легаси-скрипты частей
   кэшируются автоматически при первом же обращении — это происходит
   сразу при обычной загрузке страницы, задолго до того, как игрок
   уйдёт в офлайн, так что на практике разницы нет.
   ============================================================ */
var BUILD = '__BUILD__';
