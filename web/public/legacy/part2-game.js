...
// Второй раз на базе (candle2) TV_DEFS уже полностью пройден, и без этой
// строки playNews() слово в слово повторил бы TV_WRAP — будто выпуск
// новостей застыл во времени. Отдельная реплика для повторного визита.
var TV_WRAP2='Дело о пропавших в мебельном официально закрыто. Но родственники говорят — на записях камер до сих пор мелькает что-то у входа, после закрытия.';
...
function playNews(){
  ...
  var wrapSeen=window.Progress&&Progress.isCompleteSync('tv_wrap_seen');
  var wrapLine=wrapSeen?TV_WRAP2:TV_WRAP;
  if(!seg&&window.Progress)Progress.markComplete('tv_wrap_seen');
  var missing=!!seg,text=seg?seg.line:wrapLine,person=seg;
...
function makeFriend(hue){
  ...
  // ссылки на части для покачивания в такт музыки (см. updateFriendsDance) —
  // Этап 26, "тусовка" с несколькими друзьями разом
  g.userData.torso=torso;g.userData.head=head;g.userData.arms=arms;
  g.userData.baseTorsoY=torso.position.y;g.userData.baseHeadY=head.position.y;
  return g;
}
var FRIEND_DEFS=[
  {key:'p1_mission1',name:'Игорь',color:0x5a7a3a,greet:'Спасибо, что нашёл меня там...',
   backstory:'Я работал тут кладовщиком. Однажды после смены свет замигал, и я не смог найти дверь — а магазин будто стал больше, чем был.'},
  {key:'p1_mission2',name:'Марина',color:0x8a4a6a,greet:'Даже не думала, что выберусь оттуда.',
   backstory:'Зашла купить кресло к дню рождения. Кассирша сказала подождать за примерочными... я ждала там, кажется, очень долго.'}
];
function rescuedNamesList(){
  // используется в раскрытии Мебельщика в тронном зале (TT.reveal2) —
  // называем реально спасённых игроком, а не жёстко прибитый список,
  // чтобы фраза не врала, если игрок нашёл не всех
  if(!window.Progress)return '';
  var names=FRIEND_DEFS.filter(function(f){return Progress.isCompleteSync(f.key);})
...
function spawnFriends(){
  ...
  FRIEND_DEFS.forEach(function(f){
    ...
    g.userData.friendKey=f.key;
    g.userData.friendBackstory=f.backstory;
    addObj(g);
    FRIENDS.push({g:g,ph:Math.random()*6.28,baseY:g.rotation.y});
    names.push(f.name);
...
// Тусовка под музыку — Этап 26, то, чего не хватало: раньше друзья на
// базе просто стояли столбами. Пока играет ReunionMusic, каждый мягко
// покачивается в такт (свой сдвиг фазы, чтобы не двигались синхронно,
// как солдаты) — торс/голова вверх-вниз, руки в противофазе, лёгкий
// разворот корпуса. Без полноценного скелета/костей — тот же приём,
// что и walkCycle() в part1.html, просто по амплитуде другой ритм.
var danceT=0;
var DANCE_BEAT=60/88;   // тот же темп, что у ReunionMusic (BPM=88) — своя константа,
                        // т.к. BEAT внутри ReunionMusic не наружу не торчит
function updateFriendsDance(dt){
  if(!FRIENDS.length)return;
  var on=window.ReunionMusic&&ReunionMusic.isOn();
  danceT+=dt*(on?1:0.4);   // без музыки фаза почти замирает, не дёргается рывком
  var beat=2*Math.PI/DANCE_BEAT;   // один полный цикл покачивания — один такт
  FRIENDS.forEach(function(fr){
    var g=fr.g,ud=g.userData;
    var target=on?1:0;
    fr.amp=(fr.amp||0)+(target-(fr.amp||0))*0.06;
    if(fr.amp<0.01&&!on){
      // возврат в состояние покоя, без остаточного дрожания
      if(ud.torso)ud.torso.position.y=ud.baseTorsoY;
      if(ud.head)ud.head.position.y=ud.baseHeadY;
      if(ud.arms)ud.arms.forEach(function(a){a.rotation.x=0;});
      g.rotation.y=fr.baseY;
      return;
    }
    var ph=danceT*beat+fr.ph;
    var bob=Math.sin(ph)*0.05*fr.amp;
    if(ud.torso)ud.torso.position.y=ud.baseTorsoY+bob;
    if(ud.head)ud.head.position.y=ud.baseHeadY+bob*1.2;
    if(ud.arms)ud.arms.forEach(function(a,ai){
      a.rotation.x=Math.sin(ph+(ai?Math.PI:0))*0.5*fr.amp;
    });
    g.rotation.y=fr.baseY+Math.sin(ph*0.5)*0.35*fr.amp;
  });
}
...
function cnAct(){
  ...
  if(nCn==='companion'){
    if(nCompanionTarget){
      var ud=nCompanionTarget.userData;
      companionSetLead(nCompanionTarget,ud.friendName);
      showMsg(ud.friendName+' идёт с тобой.',2.2);
      // предыстория — один раз на друга, при первом «позвать с собой»,
      // закрывает KEN-33 (спасённые как полноценные персонажи, не молчаливые статисты)
      var bkKey='backstory_'+ud.friendKey;
      if(ud.friendBackstory&&window.Progress&&!Progress.isCompleteSync(bkKey)){
        Progress.markComplete(bkKey);
        setTimeout(function(){showSub(ud.friendName,ud.friendBackstory,5.5);},900);
      }
    }
    return;
  }
...
var TT={line1:3.2, rise:6.4, walk:7.6, shot:12.4, fall:12.7,
        jump:14.6, look:16.2, q1:18.6, q2:21.6,
        reveal1:25.4, reveal2:29.4, reveal3:33.6, follow:37.6};
function startThrone(){
  PHASE='throne';buildThrone();
  P.x=17;P.z=6;P.y=0;P.vy=0;P.yaw=Math.PI;P.pitch=0;P.eye=EYE_H;P.floorY=0;
  camera.rotation.z=0;camera.up.set(0,1,0);
  unstick();dead=false;mRun=false;syncBtn('brun',false);
  // Тронный зал — отдельная срежиссированная сцена со своим спутником
  // (Синий), затаскивать туда же ведомого друга с базы не для чего
  // и негде — но раньше он просто молча пропадал (updateCompanion()
  // не вызывается ни в updateThrone(), ни в secret2). Явно отпускаем
  // его с репликой, вместо тихого исчезновения.
  if(COMPANION.active){
    var leftName=COMPANION.name;
    companionStop();
    showSub(leftName,'Дальше я с тобой не пойду. Жди меня здесь, ладно?',3.4);
  }
...
function updateThrone(dt){
  ...
  // Раскрытие — раньше «союзник» отделывался одной фразой «я знаю, куда
  // идти», и тем, кого игрок спас на 1-2 этажах, тут вообще не находилось
  // места. Три реплики вместо одной: кто такие мебельщики (их много, не
  // один), что стало с реально спасёнными игроком (rescuedNamesList() —
  // не жёстко прибитые имена), и что упавший здесь — не главный.
  if(t>TT.reveal1&&TR2.step===11){
    TR2.step=12;
    showSub('Синий','Мы — не одна тварь. Нас много, у каждого свой этаж, свой отдел.',4.2);
    sayE('Мы не одна тварь. Нас много, у каждого свой этаж, свой отдел.');
  }
  if(t>TT.reveal2&&TR2.step===12){
    TR2.step=13;
    var rn=rescuedNamesList();
    var line2=rn?('Тех, кого ты нашёл — '+rn+' — забрали через щели между витринами. Магазин здесь не заканчивается там, где должен.')
                 :'Людей забирают через щели между витринами. Магазин здесь не заканчивается там, где должен.';
    showSub('Синий',line2,5.2);
    sayE(line2);
  }
  if(t>TT.reveal3&&TR2.step===13){
    TR2.step=14;
    showSub('Синий','Тот, кто лежит сейчас — не главный. Он просто нашёл тебя первым. Но выход есть, и я его знаю.',5.0);
    sayE('Тот, кто лежит сейчас, не главный. Он просто нашёл тебя первым. Но выход есть, и я его знаю.');
  }
  if(t>TT.follow&&TR2.step===14){
    TR2.step=15;TR2.walking=true;
...
function cn2Act(){
  ...
  if(nCn2==='companion'){
    if(nCompanionTarget){
      var ud2=nCompanionTarget.userData;
      companionSetLead(nCompanionTarget,ud2.friendName);
      showMsg(ud2.friendName+' идёт с тобой.',2.2);
      var bkKey2='backstory_'+ud2.friendKey;
      if(ud2.friendBackstory&&window.Progress&&!Progress.isCompleteSync(bkKey2)){
        Progress.markComplete(bkKey2);
        setTimeout(function(){showSub(ud2.friendName,ud2.friendBackstory,5.5);},900);
      }
    }
    return;
  }
...
var VOICE_LINES={
  ...
  // раскрытие в тронном зале (Этап 28) — вторая реплика с динамическими
  // именами записана во всех 4 вариантах, которые может собрать
  // rescuedNamesList() из текущего FRIEND_DEFS (оба/Игорь/Марина/никто)
  'Мы не одна тварь. Нас много, у каждого свой этаж, свой отдел.':'siniy_15',
  'Тех, кого ты нашёл — Игорь и Марина — забрали через щели между витринами. Магазин здесь не заканчивается там, где должен.':'siniy_16a',
  'Тех, кого ты нашёл — Игорь — забрали через щели между витринами. Магазин здесь не заканчивается там, где должен.':'siniy_16b',
  'Тех, кого ты нашёл — Марина — забрали через щели между витринами. Магазин здесь не заканчивается там, где должен.':'siniy_16c',
