...
function talkToMan(){
  ...
  setTimeout(function(){showSub('Вы','Да, я охранник. Что-то случилось?',3.0);say('Да, я охранник.',0.9,0.6);},3200);
  setTimeout(function(){
...
function smellBed(){
  ...
  showSub('Вы','Да... реально пахнет как мясо.',2.6);
  say('Да. Реально пахнет как мясо.',0.85,0.55);
...
function startRoom(){
  ...
  Music.stop();
  setTimeout(function(){
    showSub('Вы','Фух... убежал.',2.6);
    say('Фух. Убежал.',0.9,0.55);
  },1800);
}
...
function prAct(){
  ...
  if(t==='key'){
    ...
    showSub('Вы','Ключ. Прямо у решётки. Он что, обронил?',3.0);
    say('Ключ. Прямо у решётки.',0.85,0.55);
  }
  else if(t==='cage'){
    ...
    if(s.hasKey){
      ...
      showSub('Вы','Маленький ключик. В книге вырезана дырка.',3.0);
      if(PR.keys===PR.keysNeed)setTimeout(function(){showMsg('Все ключики собраны',2.4);},1200);
    }else{
      showSub('Вы','Просто книги.',1.8);
    }
  }
  else if(t==='safeKeys'){
    ...
    setTimeout(function(){
      showSub('Вы','Почему он так любит сейфы?',2.8);
      say('Почему он так любит сейфы?',0.85,0.55);
    },4600);
  }
  else if(t==='safeCode'){
    if(PR.safeCodeOpen){
      if(!PR.hasDriver){
        ...
        showSub('Вы','Отвёртка. Он держал в сейфе... отвёртку.',3.4);
        say('Он держал в сейфе отвёртку.',0.85,0.55);
      }
      return;
    }
    openKeypad();
...
function startPrison(){
  ...
  PR.intro=0.001;
  setTimeout(function(){showSub('Вы','Где я...',2.2);say('Где я.',0.8,0.5);},1600);
  setTimeout(function(){questShow('Выберитесь из клетки','Ключ где-то рядом');},4000);
  setTimeout(function(){showSub('Вы','Он ходит там. Надо тихо.',2.8);},5200);
}
...
function updateVent(dt){
  ...
  if(VT.spider&&!VT.dead){
    ...
    if(d<3.2&&!VT.warned){
      VT.warned=true;
      showSub('Вы','Там что-то шевелится.',2.4);
      sndSkitter();
    }
...
function ventHit(){
  ...
  if(VT.hits>=3){
    ...
    sndCrunch();
    setPrompt('',false);
    showSub('Вы','Ненавижу пауков.',2.4);
    say('Ненавижу пауков.',0.85,0.55);
...
function startWarehouse(){
  ...
  setTimeout(function(){showSub('Вы','Сколько же их...',2.6);say('Сколько же их.',0.8,0.5);},1600);
...
function assembleMap(){
  ...
  setTimeout(function(){
    ...
    showSub('Вы','Теперь понятно, куда идти.',2.6);
    spawnArrow();
  },5000);
}
...
function startHall(){
  ...
  setTimeout(function(){showSub('Вы','Какой высокий...',2.4);say('Какой высокий.',0.8,0.5);},1800);
  setTimeout(function(){
    showSub('Вы','Их что, двое? И они... огромные.',3.2);
    say('Их двое. И они огромные.',0.8,0.5);
  },6000);
  setTimeout(function(){
...
function startShelter(){
  ...
  document.getElementById('black').style.opacity='0';
  document.getElementById('dream').style.display='none';
  setTimeout(function(){showSub('Вы','Улица... я на улице.',2.8);say('Улица. Я на улице.',0.85,0.55);},2000);
...
function updateShelter(dt){
  ...
  if(!SH2.revealed&&P.pitch>0.34&&SH2.t>5){
    ...
    showSub('Вы','Это... потолок. Это нарисовано.',3.6);
    say('Это потолок. Это нарисовано.',0.78,0.48);
    sndDread();
    setTimeout(function(){
      showSub('Вы','Тут был приют? Нет... тут просто продавали детские вещи.',4.6);
      say('Тут просто продавали детские вещи.',0.8,0.5);
    },4200);
...
function shelterAct(){
  if(nSh==='slide'){
    if(!SH2.revealed){
      showSub('Вы','Сначала осмотрюсь.',2.0);
      return;
    }
    setPrompt('',false);
    showSub('Вы','С этой трубой что-то не так.',2.6);
    say('С этой трубой что-то не так.',0.8,0.5);
    setTimeout(function(){
      document.getElementById('black').style.transition='opacity 0.9s';
      document.getElementById('black').style.opacity='1';
    },2400);
...
function startTube(){
  ...
  setTimeout(function(){showSub('Вы','Что-то долго.',2.2);},4200);
  setTimeout(function(){showSub('Вы','Почему я всё ещё еду?!',2.6);say('Почему я всё ещё еду?',1.15,0.8);},7600);
  setTimeout(function(){showSub('Вы','Это не горка. Это не горка!',3.0);say('Это не горка!',1.25,0.85);},11200);
}
...
function startOldStore(){
  ...
  document.getElementById('black').style.opacity='0';
  Music.stop();
  setTimeout(function(){showSub('Вы','Где... где я теперь.',2.8);say('Где я теперь.',0.78,0.48);},2400);
...
function osAct(){
  ...
  if(nOs.t==='torch'){
    ...
    sndPickup();
    showSub('Вы','О! Новый фонарик! Настоящий, не то что тот, старый.',3.6);
    say('О. Новый фонарик.',0.95,0.7);
...
function nextRoom(){
  ...
  document.getElementById('black').style.transition='opacity 1.4s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Опять склад? Тот же самый?',2.8);say('Опять склад.',0.8,0.5);},1800);
  setTimeout(function(){showMsg('Вопрос другой. Читай внимательно.',3.4);},5200);
}
function finishOldStore(){
  camera.up.set(0,1,0);camera.rotation.z=0;
  showSub('Вы','Ответил правильно. Значит, я не человек?',3.2);
...
function startGas(){
  ...
  document.getElementById('black').style.opacity='0';
  sndHiss();
  setTimeout(function(){showSub('Вы','Что это... газ?',2.4);say('Что это. Газ.',0.85,0.55);},1400);
  setTimeout(function(){showSub('Вы','Не могу... стоять...',2.6);say('Не могу стоять.',0.6,0.4);},5200);
}
...
function startDream1(){
  ...
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Стоп. Это же... первый этаж.',3.0);say('Это же первый этаж.',0.8,0.5);},2600);
...
function startWake(){
  ...
  document.getElementById('black').style.transition='opacity 1.8s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Опять... опять этот сон.',2.8);say('Опять этот сон.',0.72,0.45);},2200);
  setTimeout(function(){
    showSub('Вы','То, что позади — может вернуться.',3.2);
    say('То что позади может вернуться.',0.72,0.45);
  },6000);
}
...
function startCandleRoom(){
  ...
  document.getElementById('black').style.transition='opacity 2.0s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Живой... кажется.',2.4);say('Живой. Кажется.',0.7,0.45);},2400);
  setTimeout(function(){showSub('Вы','Свечи. Кто-то их зажёг.',2.8);say('Свечи. Кто-то их зажёг.',0.8,0.5);},6400);
...
function startFork(){
  ...
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){
    showSub('Вы','Это же... тот самый грузовик.',3.0);
    say('Это тот самый грузовик.',0.8,0.5);
  },2600);
...
function fkAct(){
  if(nFk==='left'){
    if(!FK.tried){
      FK.tried=true;
      showSub('Вы','Заперто. И заколочено снаружи.',2.8);
      say('Заперто. И заколочено.',0.82,0.5);
      sndCreak();
...
function updateUV(dt){
  ...
  UV.petals.forEach(function(pet,i){
    ...
    UV.bulbs[i].material.color.setHex(lit?0xbfa8ff:0x3a3540);
  });
...
function startUVStore(){
  ...
  document.getElementById('black').style.transition='opacity 1.6s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Ещё один склад. Сколько их тут.',3.0);},2200);
...
function usAct(){
  ...
  if(nUs.t==='gun'){
    ...
    showSub('Вы','Вот он. УФ-излучатель.',2.8);
    say('Вот он. Ультрафиолетовый.',0.85,0.55);
...
function huntAct(n){
  if(n.t==='ladder'){
    ...
    setTimeout(function(){
      ...
      sndCrash();
      showSub('Вы','Лестница!.. Обратно уже не выйти.',3.0);
    },700);
    document.getElementById('black').style.transition='opacity 1.4s';
...
function startStation(){
  ...
  document.getElementById('dmg').style.opacity='0';
  questHide();
  setTimeout(function(){showSub('Вы','Это что, станция? Внутри горы?',3.2);say('Станция. Внутри горы.',0.8,0.5);},2400);
...
function cordTake(){
  ...
  sndPickup();
  showSub('Вы','Шнур. Надо дотащить его до центра.',3.0);
...
function startToyHouse(){
  ...
  document.getElementById('black').style.transition='opacity 1.4s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Игрушки. Все на местах.',2.6);},1800);
...
function toyOpenExit(){
  ...
  sndCreak();
  showSub('Вы','Открылось. И свет загорелся.',3.2);
...
function startUVHouse(){
  ...
  document.getElementById('black').style.transition='opacity 1.4s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Тут ничего не видно. Совсем.',2.8);},1800);
...
function uvOpenInner(){
  ...
  sndCreak();questDone();
  showSub('Вы','Метки сошлись. Дверь поднялась.',3.0);
  setTimeout(function(){
...
function uvOpenExit(){
  ...
  sndCreak();questDone();
  showSub('Вы','Плита сработала. И свет дали.',3.0);
...
function startThrone(){
  ...
  setTimeout(function(){showSub('Вы','Это... тронный зал?',2.6);},2200);
}
function updateThrone(dt){
  ...
  if(t>TT.look&&TR2.step===8){
    ...
    showSub('Вы','Ты точно мебельщик?',2.6);
  }
...
function answerA(){
  showSub('Вы','Я пришёл узнать, что здесь произошло.',3.0);
  setTimeout(function(){
    showSub('Синий','Зачем?',2.0);sayE('Зачем?');
  },3200);
  setTimeout(function(){
    showSub('Вы','Сотрудники здесь пропали. Мне нужно узнать, что произошло.',4.2);
  },5400);
  setTimeout(function(){
...
function answerB(){
  showSub('Вы','Я сам жалею, что сюда пришёл.',3.0);
  setTimeout(function(){
...
function startBase(){
  ...
  setTimeout(function(){showSub('Вы','База... Значит, вас тут несколько.',3.2);},6600);
...
function startCandleReturn(){
  ...
  questHide();
  setTimeout(function(){
    showSub('Вы','Это же та комната. Со свечами.',3.0);
  },2200);
}
...
function updateCandleReturn(dt){
  ...
  if(t>CT2.l1&&CN2.step===0){
    CN2.step=1;
    showSub('Вы','Стоп. Лестница же упала. Обратно через вентиляцию не выйти.',4.4);
  }
...
function cn2Act(){
  ...
  showSub('Вы','Открылся. Там темно.',2.8);
  setTimeout(function(){
    document.getElementById('black').style.transition='opacity 1.2s';
    document.getElementById('black').style.opacity='1';
  },2600);
...
function startBasement(){
  ...
  document.getElementById('black').style.transition='opacity 2.0s';
  document.getElementById('black').style.opacity='0';
  setTimeout(function(){showSub('Вы','Подвал. Тут ничего не видно.',3.0);},2200);
  setTimeout(function(){
...
function bsOpenExit(){
  ...
  sndCreak();questDone();
  showSub('Вы','Открылось. Дальше — вниз.',3.0);
...
function startDome(back){
  ...
  DomeMusic.start();
  if(!back){
    setTimeout(function(){showSub('Вы','Опять приют. Только... под землёй.',3.4);say('Опять приют. Под землёй.',0.78,0.48);},2600);
    setTimeout(function(){
      showSub('Вы','Наверху что-то ходит. Железное.',3.2);
      sndSteel();
    },7200);
  }
  if(back){
...
function updateDome(dt){
  ...
  if(!DM.reached&&Math.hypot(P.x,P.z)<4.2&&DM.t>4){
    ...
    showSub('Вы','Это сердце приюта. Отсюда управляют всем.',4.0);
    say('Это сердце приюта. Отсюда управляют всем.',0.8,0.5);
    setTimeout(function(){
      var tgt=houseTarget();
      if(tgt)questShow(tgt.task,tgt.hint);
