...
#    2. проставляет номер сборки в version.json и в sw.js;
#    3. проверяет, что все обязательные файлы на месте;
#    4. АТОМАРНО переставляет символическую ссылку www.
#       Игрок в этот момент либо видит старую версию, либо новую —
#       промежуточного состояния с половиной файлов не бывает.
#    5. Старые версии остаются рядом: откат — одна команда.
#
...
for f in index.html part1.html part2.html sw.js version.json manifest.webmanifest \
         vendor/three.min.js js/detail.js js/updater.js js/profile.js js/progress.js js/coins.js js/inventory.js js/mapview.js js/controls.js \
         icons/icon-192.png icons/icon-512.png; do
  [ -s "$f" ] || die "в сборке нет файла $f (или он пустой) — выкладка отменена"
done
# three.js должен быть настоящим, а не страницей ошибки от прокси
[ "$(stat -c%s vendor/three.min.js)" -gt 400000 ] || die "vendor/three.min.js подозрительно мал"
...
curl -fsS -o /dev/null http://127.0.0.1:8091/vendor/three.min.js || die "three.js не отдаётся"
